...
describe("WebAuthService", () => {
	...
	describe("login", () => {
		...
		it("should use package.json values for redirect URI with default sign-in endpoint", async () => {
			...
			const expectedUrl =
				"https://api.test.com/extension/sign-in?state=746573742d72616e646f6d2d6279746573&auth_redirect=vscode%3A%2F%2Fallquixotic.crc"
			expect(mockOpenExternal).toHaveBeenCalledWith(
				expect.objectContaining({
					toString: expect.any(Function),
				}),
		...
		it("should use provider signup URL when useProviderSignup is true", async () => {
			...
			const expectedUrl =
				"https://api.test.com/extension/provider-sign-up?state=746573742d72616e646f6d2d6279746573&auth_redirect=vscode%3A%2F%2Fallquixotic.crc"
			expect(mockOpenExternal).toHaveBeenCalledWith(
				expect.objectContaining({
					toString: expect.any(Function),
				}),
