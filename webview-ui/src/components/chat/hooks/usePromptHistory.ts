...
export const usePromptHistory = ({
	...
	const filteredPromptHistory = useMemo(() => {
		...
		if (conversationPrompts?.length || queuedPrompts?.length || persistedPromptHistory.length) {
			const activeConversationHistory = [...(conversationPrompts ?? []), ...(queuedPrompts ?? [])]
	...
	}, [clineMessages, messageQueue, persistedPromptHistory, taskHistory, cwd, mergePromptHistories])
