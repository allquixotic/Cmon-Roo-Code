...
describe("getVisibleProviderOrLog", () => {
	...
	it("logs and returns undefined if no provider found", () => {
		...
		expect(mockOutputChannel.appendLine).toHaveBeenCalledWith("Cannot find any visible CRC instances.")
	})
})
...
describe("registerCommands handlers", () => {
	...
	let mockProvider: { postMessageToWebview: Mock; contextProxy: { getValue: Mock } }
	...
	beforeEach(() => {
		...
		mockProvider = {
			postMessageToWebview: vi.fn().mockResolvedValue(undefined),
			contextProxy: { getValue: vi.fn().mockReturnValue("sidebar") },
		}
