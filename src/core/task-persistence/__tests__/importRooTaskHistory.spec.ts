...
describe("importRooTaskHistory", () => {
		...
		getConfigurationMock.mockImplementation((section?: string) => {
			...
			return {
				get: vi.fn().mockImplementation(() => {
					...
					// The fork reads its own custom storage path via Package.name (= "crc").
					if (resolvedSection === "crc") {
						return zoo
					}
	...
	it("resolves Roo and Zoo storage roots from extension domains and configured custom paths", async () => {
		...
		// The active (destination) extension domain derives from the fork's publisher/name.
		expect(result.zooExtensionDomain).toBe("allquixotic.crc")
