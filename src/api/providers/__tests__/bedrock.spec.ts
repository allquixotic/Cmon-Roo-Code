...
describe("AwsBedrockHandler", () => {
	...
	describe("Claude 4.7+ adaptive thinking (Opus 4.7 / Opus 4.8)", () => {
		...
		it("should send adaptive thinking with effort xhigh for Claude Opus 4.7 when reasoning is enabled", async () => {
			...
			// output_config.effort lives at the TOP LEVEL of the payload, alongside
			// additionalModelRequestFields. Effort is derived from the default reasoning
			// budget (8192 → "medium") via the fork's mapReasoningBudgetToBedrockEffort.
		...
		it("should send adaptive thinking with effort xhigh for Claude Opus 4.8 when reasoning is enabled", async () => {
			...
			// output_config.effort lives at the TOP LEVEL of the payload (fork shape),
			// derived from the default reasoning budget (8192 → "medium").
			expect(commandArg.output_config).toEqual({ effort: "medium" })
			// Sampling parameters are still rejected on 4.8 — temperature must be absent.
			expect(commandArg.inferenceConfig?.temperature).toBeUndefined()
		})

		it("should send adaptive thinking with derived effort for Claude Sonnet 5 when reasoning is enabled", async () => {
