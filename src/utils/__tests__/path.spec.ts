...
/**
 * Set the value returned for the `workspace.rootResolution` setting in tests.
 *
 * Production reads this via `getConfiguration(Package.name)`, and the fork's
 * `Package.name` is "crc" (see src/package.json), so the mock must key off the
 * "crc" config section — not the user-facing "zoo-code" brand.
 *
...
function setRootResolution(value: "activeEditor" | "firstFolder" | undefined) {
	mockWorkspace.getConfiguration = (section?: string) => ({
		get: (key: string, defaultValue?: unknown) => {
			if (section === "crc" && key === "workspace.rootResolution") {
