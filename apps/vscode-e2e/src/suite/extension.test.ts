...
suite("Zoo Code Extension", function () {
	...
	test("Commands should be registered", async () => {
		...
		const commands = new Set((await vscode.commands.getCommands(true)).filter((cmd) => cmd.startsWith("crc")))

		for (const command of expectedCommands) {
			assert.ok(commands.has(`crc.${command}`), `Command ${command} should be registered`)
		}
	})
})
